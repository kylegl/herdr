...
fn collect_agent_panel_entries_with_runtimes(
    ...
    app.workspaces
        ...
        .flat_map(|(ws_idx, ws)| {
            ...
            let workspace_label = app.workspace_display_name_from(ws, terminal_runtimes);
...
fn workspace_row_height(app: &AppState, ws: &crate::workspace::Workspace, indented: bool) -> u16 {
    let (state, seen) = app.workspace_aggregate_state(ws);
...
fn space_aggregate_state(app: &AppState, key: &str) -> (AgentState, bool) {
    app.workspaces
        ...
        .map(|ws| app.workspace_aggregate_state(ws))
...
pub(super) fn render_sidebar_collapsed(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    for (visible_idx, ws) in app.workspaces.iter().enumerate() {
        ...
        let (agg_state, agg_seen) = app.workspace_aggregate_state(ws);
...
pub(super) fn render_sidebar(
    ...
    ) || (app.sidebar_navigation.is_none()
...
fn render_workspace_list(
    ...
    for card in cards {
        ...
        let label = app.workspace_display_name_from(ws, terminal_runtimes);
