...
#[tokio::test]
async fn retained_surface_keeps_last_complete_synchronized_frame() {
    let (mut server, _client_rx, pane_id) = retained_test_server(b"complete");
    server.render_and_stream();
    let runtime = server
        .app
        .state
        .runtime_for_pane_in_workspace(&server.app.terminal_runtimes, 0, pane_id)
        .expect("pane runtime");
    runtime.test_process_pty_bytes(b"\x1b[?2026h\x1b[2J\x1b[Hpartial");
    ...
    assert!(frame_text(&retained.frame).contains("complete"));
    assert!(!frame_text(&retained.frame).contains("partial"));
    ...
    let runtime = server
        ...
        .runtime_for_pane_in_workspace(&server.app.terminal_runtimes, 0, pane_id)
        .expect("pane runtime");
    runtime.test_process_pty_bytes(b"\x1b[2J\x1b[Hfinished\x1b[?2026l");
    assert!(!runtime.synchronized_output_active());
    assert!(
        !server.render_retained_pane_surface_and_stream(&HashSet::from([pane_id])),
        "the completed synchronized batch must use one coherent full render"
    );
    shutdown_test_runtimes(&mut server);
}

#[tokio::test]
async fn retained_surface_completes_each_synchronized_pane_independently() {
    let (mut server, _client_rx, first_pane) = retained_test_server(b"first");
    server.render_and_stream();
    let mut second_workspace = crate::workspace::Workspace::test_new("second-sync");
    let second_pane = second_workspace.tabs[0].root_pane;
    second_workspace.insert_test_runtime(
        second_pane,
        crate::terminal::TerminalRuntime::test_with_screen_bytes(80, 24, b"second"),
    );
    server.app.state.workspaces.push(second_workspace);
    for pane_id in [first_pane, second_pane] {
        let (workspace_index, _) = server.app.find_pane(pane_id).expect("sync pane");
        server
            .app
            .state
            .runtime_for_pane_in_workspace(&server.app.terminal_runtimes, workspace_index, pane_id)
            .expect("sync runtime")
            .test_process_pty_bytes(b"\x1b[?2026hpartial");
    }
    assert!(
        server.render_retained_pane_surface_and_stream(&HashSet::from([first_pane, second_pane,]))
    );
    assert_eq!(
        server.synchronized_panes_pending_full_render,
        HashSet::from([first_pane, second_pane])
    );

    server
        .app
        .state
        .runtime_for_pane_in_workspace(&server.app.terminal_runtimes, 0, first_pane)
        .expect("first runtime")
        .test_process_pty_bytes(b"finished\x1b[?2026l");
    assert!(
        !server.render_retained_pane_surface_and_stream(&HashSet::from([first_pane])),
        "first completion must force a coherent frame while the second remains active"
    );
    assert_eq!(
        server.synchronized_panes_pending_full_render,
        HashSet::from([second_pane])
    );
    ...
    assert!(server.synchronized_panes_pending_full_render.is_empty());
    shutdown_test_runtimes(&mut server);
}

#[tokio::test]
async fn retained_surface_prunes_closed_synchronized_panes() {
    let (mut server, _client_rx, live_pane) = retained_test_server(b"live");
    server.render_and_stream();
    let mut closing_workspace = crate::workspace::Workspace::test_new("closing-sync");
    let closing_pane = closing_workspace.tabs[0].root_pane;
    closing_workspace.insert_test_runtime(
        closing_pane,
        crate::terminal::TerminalRuntime::test_with_screen_bytes(80, 24, b"closing"),
    );
    server.app.state.workspaces.push(closing_workspace);
    server
        .app
        .state
        .runtime_for_pane_in_workspace(&server.app.terminal_runtimes, 1, closing_pane)
        .expect("closing runtime")
        .test_process_pty_bytes(b"\x1b[?2026hpartial");
    assert!(server.render_retained_pane_surface_and_stream(&HashSet::from([closing_pane])));
    assert!(server
        .synchronized_panes_pending_full_render
        .contains(&closing_pane));

    server.app.state.workspaces.pop();
    let _ = server.render_retained_pane_surface_and_stream(&HashSet::from([live_pane]));
    assert!(!server
        .synchronized_panes_pending_full_render
        .contains(&closing_pane));
    shutdown_test_runtimes(&mut server);
...
#[test]
fn client_shell_attention_projection_keeps_source_identity() {
    let mut server = test_headless_server();
    ...
    let source_tab_id =
        crate::workspace::public_tab_id_for_number(&source_workspace_id, home.tabs[0].number);
    let source_pane_number = home
        .public_pane_number(attention_pane)
        .expect("source pane number");
    let source_pane_id =
        crate::workspace::public_pane_id_for_number(&source_workspace_id, source_pane_number);
    let work = crate::workspace::Workspace::test_new("work");
    server.app.state.workspaces = vec![home, work];
    server.app.state.ensure_test_terminals();
    ...
    server.app.state.reconcile_attention_dock();

    let attention = client_shell_attention_projection(&server.app).expect("projection");
    assert_eq!(attention.source_workspace_id, source_workspace_id);
    assert_eq!(attention.source_tab_id, source_tab_id);
    assert_eq!(attention.source_pane_id, source_pane_id);
    assert_ne!(attention.pane_id, attention.source_pane_id);
    ...
    let agent = snapshot
        ...
        .expect("attention agent");
    assert_eq!(agent.workspace_id, attention.source_workspace_id);
    assert_eq!(agent.tab_id, attention.source_tab_id);
    shutdown_test_runtimes(&mut server);
}

#[cfg(unix)]
#[test]
fn over_limit_handoff_rejection_redocks_ready_attention() {
    let mut server = test_headless_server();
    let home = crate::workspace::Workspace::test_new("attention-home");
    let attention_pane = home.tabs[0].root_pane;
    let host = crate::workspace::Workspace::test_new("host");
    let host_target = crate::app::attention_dock::AttentionHostTarget {
    ...
    server
        .app
        .state
        .set_attention_host(Some(host_target), &server.app.terminal_runtimes);
    server
        .app
        .state
        .reconcile_attention_dock_from(&server.app.terminal_runtimes);
    assert_eq!(
        server.app.find_pane(attention_pane).map(|(index, _)| index),
        Some(1)
    );
    server.app.state.prepare_attention_handoff();
    assert_eq!(
        server.app.find_pane(attention_pane).map(|(index, _)| index),
        Some(0)
    );
    let socket_path = std::env::temp_dir().join(format!(
        "herdr-over-limit-handoff-{}-{}.sock",
        std::process::id(),
        attention_pane.raw()
    ));

    let error = server
        .reject_over_limit_handoff(
            crate::server::handoff::MAX_FDS_PER_HANDOFF + 1,
            &socket_path,
        )
        .expect_err("over-limit handoff should be rejected");
...
#[test]
fn stale_attention_open_and_dismiss_cannot_mutate_the_next_item() {
    let mut server = test_headless_server();
    let first_home = crate::workspace::Workspace::test_new("first-attention");
    let first = first_home.tabs[0].root_pane;
    let second_home = crate::workspace::Workspace::test_new("second-attention");
    let second = second_home.tabs[0].root_pane;
    let host = crate::workspace::Workspace::test_new("host");
    server.app.state.workspaces = vec![first_home, second_home, host];
    server.app.state.ensure_test_terminals();
    server.app.state.active = Some(2);
    server.app.state.selected = 2;
    server.app.state.mode = crate::app::Mode::Terminal;
    let first_source_id = server
        .app
        .public_pane_id(0, first)
    ...
    for pane_id in [first, second] {
        ...
        server.app.state.make_attention_ready_for_test(pane_id);
    }
    server
        .app
        .state
        .set_attention_host(None, &server.app.terminal_runtimes);
    let (owner_control, _owner_render) = connect_test_shell(&mut server, 10, 100, 30);
    let _ = owner_control.recv().expect("owner snapshot");
    assert_eq!(
        server
            .app
            .state
            .attention_source_target()
    ...
    for (id, method) in [
        (
            ...
            api::schema::Method::AttentionOpen(api::schema::AttentionTarget {
                source_pane_id: first_source_id.clone(),
            }),
        ),
        (
            "stale-dismiss",
        ...
        let response: api::schema::ErrorResponse =
            serde_json::from_str(&response_rx.recv().expect("stale attention response"))
                .expect("stale attention error");
        ...
        assert_eq!(
            server
                ...
                .state
                .attention_source_target()
                .expect("second attention remains current")
                .public_pane_id,
            second_source_id
        );
    }
...
#[test]
fn attention_host_follows_stable_owner_and_transfers_on_disconnect() {
    let mut server = test_headless_server();
    let home = crate::workspace::Workspace::test_new("attention-home");
    let attention_pane = home.tabs[0].root_pane;
    let work = crate::workspace::Workspace::test_new("work");
    ...
    let (_owner_control, _owner_render) = connect_test_shell(&mut server, 10, 100, 30);
    let (_second_control, _second_render) = connect_test_shell(&mut server, 20, 80, 24);
    assert_eq!(server.attention_owner_client_id, Some(10));
    assert_eq!(
        server
            .app
            .find_pane(attention_pane)
            .map(|(ws_idx, _)| ws_idx),
        Some(1)
    );

    let source_tab_id = crate::workspace::public_tab_id_for_number(
        &server.app.state.workspaces[0].id,
        server.app.state.workspaces[0].tabs[0].number,
    );
    assert!(server.focus_shell_client_on_tab(20, &source_tab_id));
    assert!(!server.sync_attention_owner());
    assert_eq!(
        server
            .app
            .find_pane(attention_pane)
    ...
    assert_ne!(
        server.clients[&10]
            .shell_location
            .as_ref()
            .and_then(|location| location.focused_tab_id()),
        server.clients[&20]
            .shell_location
            .as_ref()
            .and_then(|location| location.focused_tab_id())
    ...
    server.clients.remove(&10);
    assert!(server.sync_attention_owner());
    assert_eq!(server.attention_owner_client_id, Some(20));
    assert_eq!(
        server
            .app
            .find_pane(attention_pane)
...
#[test]
fn async_worktree_completions_resynchronize_attention_to_the_owner_location() {
    let mut server = test_headless_server();
    let home = crate::workspace::Workspace::test_new("attention-home");
    let attention_pane = home.tabs[0].root_pane;
    let first = crate::workspace::Workspace::test_new("first-host");
    let second = crate::workspace::Workspace::test_new("second-host");
    server.app.state.workspaces = vec![home, first, second];
    server.app.state.ensure_test_terminals();
    server.app.state.active = Some(1);
    server.app.state.selected = 1;
    server.app.state.mode = crate::app::Mode::Terminal;
    ...
    let second_tab_id = server.app.public_tab_id(2, 0).expect("second host tab");
    assert!(server.focus_shell_client_on_tab(10, &second_tab_id));
    assert!(
        server.handle_internal_event_with_forwarding(AppEvent::WorktreeAddFinished(Box::new(
            crate::events::WorktreeAddResult {
                path: PathBuf::from("/repo/add-failed"),
                api_request: None,
                result: Err("test add failure".into()),
            }
        ),))
    );
    assert_eq!(
    ...
    let first_workspace_id = server.app.state.workspaces[1].id.clone();
    let first_tab_id = server.app.public_tab_id(1, 0).expect("first host tab");
    assert!(server.focus_shell_client_on_tab(10, &first_tab_id));
    assert!(
        server.handle_internal_event_with_forwarding(AppEvent::WorktreeRemoveFinished(Box::new(
            crate::events::WorktreeRemoveResult {
                ...
                path: PathBuf::from("/repo/remove-failed"),
                workspace: None,
                worktree: None,
                forced: false,
                api_request: None,
                result: Err("test remove failure".into()),
            }
        )))
    );
    assert_eq!(
...
#[test]
fn public_focus_resynchronizes_attention_to_the_owner_location() {
    let mut server = test_headless_server();
    let home = crate::workspace::Workspace::test_new("attention-home");
    let attention_pane = home.tabs[0].root_pane;
    let first = crate::workspace::Workspace::test_new("first-host");
    ...
    let second_workspace_id = server.app.public_workspace_id(2);
    let (respond_to, _response_rx) = std::sync::mpsc::channel();
    server.handle_api_request_with_shutdown_check(api::ApiRequestMessage {
        request: api::schema::Request {
            id: "public-focus-owner".into(),
...
#[test]
fn cross_client_tab_close_reconciles_owner_before_attention_host_sync() {
    let mut server = test_headless_server();
    let home = crate::workspace::Workspace::test_new("attention-home");
    let attention_pane = home.tabs[0].root_pane;
    let mut host = crate::workspace::Workspace::test_new("host");
    let second_tab = host.test_add_tab(Some("second"));
    server.app.state.workspaces = vec![home, host];
    server.app.state.ensure_test_terminals();
    server.app.state.active = Some(1);
    server.app.state.selected = 1;
    ...
    server
        ...
        .set_attention_host(None, &server.app.terminal_runtimes);
    let (owner_control, _owner_render) = connect_test_shell(&mut server, 10, 100, 30);
    let (other_control, _other_render) = connect_test_shell(&mut server, 20, 80, 24);
    let _ = owner_control.recv().expect("owner snapshot");
    let _ = other_control.recv().expect("other snapshot");
    let first_tab_id = server.app.public_tab_id(1, 0).expect("first host tab");
    let second_tab_id = server
        .app
        .public_tab_id(1, second_tab)
        .expect("second host tab");
    assert!(server.focus_shell_client_on_tab(10, &second_tab_id));
    assert!(server.sync_attention_owner());
    assert!(server.app.state.workspaces[1].tabs[second_tab]
        .panes
    ...
    server.handle_client_shell_api_request(
        ...
        api::ApiRequestMessage {
            request: api::schema::Request {
                id: "other-closes-owner-tab".into(),
                method: api::schema::Method::TabClose(api::schema::TabTarget {
...
#[tokio::test]
async fn attention_open_follows_source_without_moving_second_client() {
    let mut server = test_headless_server();
    let home = crate::workspace::Workspace::test_new("attention-home");
    let attention_pane = home.tabs[0].root_pane;
    let source_tab_id = crate::workspace::public_tab_id_for_number(&home.id, home.tabs[0].number);
    let work = crate::workspace::Workspace::test_new("work");
    let work_tab_id = crate::workspace::public_tab_id_for_number(&work.id, work.tabs[0].number);
    server.app.state.workspaces = vec![home, work];
    server.app.state.ensure_test_terminals();
    server.app.state.active = Some(1);
    server.app.state.selected = 1;
    ...
    let (owner_control, _owner_render) = connect_test_shell(&mut server, 10, 100, 30);
    let (second_control, _second_render) = connect_test_shell(&mut server, 20, 80, 24);
    let _ = owner_control.recv().expect("owner snapshot");
    let _ = second_control.recv().expect("second snapshot");
    let attention_source_pane_id = server
        .app
        .state
        .attention_source_target()
    ...
    assert!(
        server.handle_server_event(ServerEvent::ClientShellEndpointRequest {
            ...
            request: Box::new(api::schema::Request {
                id: "open-attention".into(),
                method: api::schema::Method::AttentionOpen(api::schema::AttentionTarget {
                    source_pane_id: attention_source_pane_id,
                }),
            }),
        })
    );
    let response_ready = server
        ...
        .recv()
        .await
        .expect("attention response ready");
    assert!(!server.handle_server_event(response_ready));
    let _ = owner_control.recv().expect("attention response");
    ...
    assert_eq!(server.app.state.docked_attention_pane(), None);
    shutdown_test_runtimes(&mut server);
}
