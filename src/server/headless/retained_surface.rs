...
impl HeadlessServer {
    ...
    pub(super) fn render_retained_pane_surface_and_stream(
        ...
        // Closed panes can disappear without another PTY event of their own. Do not retain their
        // synchronized-output completion bookkeeping indefinitely.
        let app = &self.app;
        self.synchronized_panes_pending_full_render
            .retain(|pane_id| app.find_pane(*pane_id).is_some());

        let mut synchronized_batch_completed = false;
        for source in pty_sources {
            let synchronized = self
                .app
                .find_pane(*source)
                .and_then(|(workspace_index, _)| {
            ...
            if synchronized {
                self.synchronized_panes_pending_full_render.insert(*source);
            } else if self.synchronized_panes_pending_full_render.remove(source) {
                synchronized_batch_completed = true;
            }
        }
        if synchronized_batch_completed {
            // Ghostty intentionally withholds synchronized damage from the retained render state.
            // Every pane completes independently, so publish one coherent full frame for this
            // pane even while another pane still has a synchronized batch open.
            fallback!("synchronized_batch_complete");
        }
        ...
        for source in pty_sources {
            ...
            // Another pane can wake the shared render loop while this terminal is
            // midway through a synchronized redraw. Keep its previous cells and
            // dirty rows until the redraw completes.
            if runtime.synchronized_output_active() {
                crate::render_prof::event("retained_surface.pane_synchronized");
                continue;
            }
