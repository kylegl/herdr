...
fn run_handoff_import_server(socket_path: &Path, token: &str) -> io::Result<()> {
    ...
    let result = rt.block_on(async {
        ...
        if let Some(attention) = received.manifest.attention.take() {
            app.restore_attention_handoff_state(
                crate::app::attention_dock::AttentionHandoffState {
                    queue: attention
                        .queue
                        .into_iter()
                        .map(|entry| crate::app::attention_dock::AttentionHandoffEntry {
            ...
            // Compatibility with senders predating explicit attention handoff metadata.
