...
// These generation-1 method shapes are frozen by
...
static CLIENT_SHELL_METHODS: std::sync::LazyLock<Vec<&'static str>> =
    std::sync::LazyLock::new(|| {
        let mut methods = CLIENT_SHELL_METHODS_V1.to_vec();
        methods.extend_from_slice(CLIENT_SHELL_METHOD_EXTENSIONS);
...
pub(crate) fn supports_client_shell_method_name(method: &str) -> bool {
    CLIENT_SHELL_METHODS.binary_search(&method).is_ok()
}
...
mod tests {
    ...
    fn advertised_client_shell_methods_are_sorted_unique_and_in_schema() {
        ...
        for method in CLIENT_SHELL_METHODS.iter() {
            assert!(
