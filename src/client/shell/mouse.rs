...
impl ClientShellState {
    ...
    pub(super) fn handle_mouse(&mut self, mouse: MouseEvent, outcome: &mut ClientShellInput) {
        ...
        if self.overlay.is_none()
            ...
            && mouse.kind == MouseEventKind::Down(MouseButton::Left)
        {
            let attention = self.active_attention().cloned();
            let pane_hit = attention.as_ref().and_then(|attention| {
                self.hits
                    .panes
                    .iter()
                    .find(|hit| hit.pane_id == attention.pane_id)
            });
            // The controls are painted into the top border. Borderless panes must leave the
            // first terminal row entirely to the terminal, including its mouse input.
            if let (Some(attention), Some(hit)) = (
                attention.as_ref(),
                pane_hit.filter(|hit| hit.inner_rect.y > hit.rect.y),
            ) {
                let dismiss_width = "[Dismiss]".len() as u16;
                let dismiss_x = hit
                    .rect
                    .right()
                    .saturating_sub(dismiss_width)
                    .saturating_sub(1);
                let dismiss = Rect::new(dismiss_x, hit.rect.y, dismiss_width, 1);
                if hit.rect.width >= dismiss_width.saturating_add(6)
                    && super::contains(dismiss, point)
                {
                    self.push_endpoint_method(
                        crate::api::schema::Method::AttentionDismiss(
                            crate::api::schema::AttentionTarget {
                                source_pane_id: attention.source_pane_id.clone(),
                            },
                        ),
                        outcome,
                    );
                ...
                let title_width = u16::try_from(crate::ui::display_width(&attention.title))
                    .unwrap_or(u16::MAX)
                    .saturating_add(2)
                    .min(dismiss_x.saturating_sub(hit.rect.x.saturating_add(1)));
                let title = Rect::new(hit.rect.x.saturating_add(1), hit.rect.y, title_width, 1);
                if super::contains(title, point) {
                    self.push_endpoint_method(
                        crate::api::schema::Method::AttentionOpen(
                            crate::api::schema::AttentionTarget {
                                source_pane_id: attention.source_pane_id.clone(),
                            },
                        ),
