...
#[test]
fn attention_keybindings_dispatch_advertised_endpoint_actions() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(snapshot()));
    state.set_endpoint_methods(Some(vec![
        crate::protocol::endpoint::ATTENTION_OPEN_METHOD.into(),
        crate::protocol::endpoint::ATTENTION_DISMISS_METHOD.into(),
    ]));
    state.set_endpoint_attention(
        &ClientEndpointId::Local,
        Some(crate::protocol::endpoint::EndpointAttentionProjection {
            pane_id: "pane_1".into(),
            source_workspace_id: "source".into(),
            source_tab_id: "source-tab".into(),
            source_pane_id: "source-pane".into(),
            title: "pi · source".into(),
        }),
    );
    ...
    let [ClientShellAction::Endpoint { request, .. }] = &open.actions[..] else {
        panic!("expected attention open request");
    };
    assert!(matches!(
        &request.method,
        crate::api::schema::Method::AttentionOpen(params)
            if params.source_pane_id == "source-pane"
    ));

    let mut dismiss = ClientShellInput::default();
    state.record_binding(
        crate::input::KeybindMatch::Action(crate::input::KeybindAction::DismissAttention),
        &mut dismiss,
    );
    let [ClientShellAction::Endpoint { request, .. }] = &dismiss.actions[..] else {
        panic!("expected attention dismiss request");
    };
    assert!(matches!(
        &request.method,
        crate::api::schema::Method::AttentionDismiss(params)
            if params.source_pane_id == "source-pane"
    ));
}

#[test]
fn attention_method_shapes_require_canonical_source_identity() {
    for method in [
        crate::api::schema::Method::AttentionOpen(crate::api::schema::AttentionTarget {
            source_pane_id: "pane_source".into(),
        }),
        crate::api::schema::Method::AttentionDismiss(crate::api::schema::AttentionTarget {
            source_pane_id: "pane_source".into(),
        }),
    ] {
        let value = serde_json::to_value(crate::api::schema::Request {
            id: "attention-shape".into(),
            method,
        })
        .expect("serialize attention request");
        assert_eq!(
            value["params"]["source_pane_id"],
            serde_json::Value::String("pane_source".into())
        );
    }
}
