...
#[test]
fn desktop_workspace_navigation_reveals_with_collapsed_group_rows() {
    let mut projected = snapshot();
    projected.workspaces[0].worktree = Some(ClientShellWorktree {
        key: "repo".into(),
        label: "repo".into(),
    ...
    for index in 2..=8 {
        ...
        workspace.label = format!("workspace-{index}");
        workspace.focused = false;
        workspace.worktree = (index <= 4).then(|| ClientShellWorktree {
            key: "repo".into(),
            label: "repo".into(),
            is_linked_worktree: true,
        });
    ...
    state.set_pane_surface(surface());
    state.mode = ClientShellMode::Navigate;
    state.select_navigate_workspace(ClientEndpointId::Local, "ws_1".into());
    state
        .compose(106, 10)
        .expect("collapsed overflowing sidebar");

    for _ in 0..4 {
        state.handle_input_bytes(b"\x1b[B");
        state
            .compose(106, 10)
            .expect("revealed collapsed workspace");
        let selected = state.navigate_workspace_id.as_deref().expect("selection");
        assert!(
            state
                .hits
                ...
                .iter()
                .any(|hit| hit.workspace_id == selected),
            "{selected} should remain visible when hidden group children do not consume rows"
        );
    }
...
fn sidebar_navigation_snapshot() -> ClientShellSnapshot {
    let mut projected = snapshot();
    let mut second_pane = projected.panes[0].clone();
    second_pane.pane_id = "pane_2".into();
    second_pane.focused = false;
...
#[test]
fn directional_sidebar_navigation_enters_sections_keeps_identity_and_focuses_agent() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(sidebar_navigation_snapshot()));
    state.set_pane_surface(surface());
    state.compose(106, 30).expect("initial frame geometry");

    assert!(state.handle_input_bytes(&[0x02]).actions.is_empty());
    let enter_sidebar = state.handle_input_bytes(b"h");
    assert!(enter_sidebar.actions.is_empty());
    assert_eq!(state.mode, ClientShellMode::Navigate);
    assert_eq!(state.navigate_endpoint_id, Some(ClientEndpointId::Local));
    assert_eq!(state.navigate_workspace_id.as_deref(), Some("ws_1"));
    assert!(state.navigate_agent_pane_id.is_none());

    assert!(state.handle_input_bytes(b"j").actions.is_empty());
    assert_eq!(state.navigate_agent_pane_id.as_deref(), Some("pane_1"));
    assert!(state.handle_input_bytes(b"\x1b[B").actions.is_empty());
    assert_eq!(state.navigate_agent_pane_id.as_deref(), Some("pane_2"));

    let mut reordered = sidebar_navigation_snapshot();
    reordered.revision += 1;
    reordered.agents.swap(0, 1);
    state.set_snapshot(Box::new(reordered));
    let mut reordered_surface = surface();
    reordered_surface.projection_revision += 1;
    state.set_pane_surface(reordered_surface);
    assert_eq!(
        state.navigate_agent_pane_id.as_deref(),
        Some("pane_2"),
        "agent reorder must not retarget a stable selection"
    );

    let frame = state.compose(106, 30).expect("selected agent frame");
    let selected = state
        .hits
        .agents
        .iter()
        .find(|(_, pane_id)| pane_id == "pane_2")
        .expect("selected agent should be visible")
        .0;
    let selected_cell =
        &frame.cells[usize::from(selected.y) * usize::from(frame.width) + usize::from(selected.x)];
    let mut expected = ratatui::buffer::Cell::default();
    expected.set_bg(state.config.palette.selection_bg);
    assert_eq!(
        selected_cell.bg,
    ...
    let [ClientShellAction::Endpoint { request, .. }] = &focus.actions[..] else {
        panic!("selected agent should focus by endpoint method");
    };
    assert!(matches!(
        &request.method,
        crate::api::schema::Method::PaneFocus(target) if target.pane_id == "pane_2"
...
#[test]
fn directional_sidebar_navigation_moves_back_to_spaces_and_right_to_pane() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(sidebar_navigation_snapshot()));
    state.set_pane_surface(surface());
    state.compose(106, 30).expect("initial frame geometry");
    ...
    assert!(state.navigate_agent_pane_id.is_some());
    assert!(state.handle_input_bytes(b"k").actions.is_empty());
    assert!(state.navigate_agent_pane_id.is_none());
    assert_eq!(state.navigate_workspace_id.as_deref(), Some("ws_1"));
...
#[test]
fn workspace_picker_directional_right_dispatches_without_entering_sidebar_focus() {
    for key in [
        ...
        assert_eq!(state.mode, ClientShellMode::Navigate);
        assert!(!state.sidebar_section_focus);

        let right = state.handle_raw_events(vec![RawInputEvent::Key(key)]);
        assert!(
            matches!(
                &right.actions[..],
                [ClientShellAction::Endpoint { request, .. }]
            ...
            "unexpected workspace-picker right action: {:?}",
            right.actions
        );
        assert_eq!(state.mode, ClientShellMode::Navigate);
        assert!(!state.sidebar_section_focus);
...
#[test]
fn remote_workspace_actions_require_activation_before_mutation() {
    for (code, modifiers) in [
        ('n', KeyModifiers::SHIFT),
        ('w', KeyModifiers::SHIFT),
        ('d', KeyModifiers::SHIFT),
        ('g', KeyModifiers::SHIFT),
        ('u', KeyModifiers::empty()),
        ('y', KeyModifiers::empty()),
    ] {
        let mut config = Config::default();
        config.keys.open_worktree = crate::config::BindingConfig::one("prefix+u");
        config.keys.remove_worktree = crate::config::BindingConfig::one("prefix+y");
        let mut state = ClientShellState::new(ClientShellConfig::from_config(&config));
        state.set_snapshot(Box::new(snapshot()));
        let remote_id = ClientEndpointId::Ssh(crate::client::endpoint::ProfileId::generate());
        let mut remote = state.endpoints[0].clone();
        remote.endpoint_id = remote_id.clone();
        state.endpoints.push(remote);
        state.mode = ClientShellMode::Navigate;
        state.select_navigate_workspace(remote_id.clone(), "ws_1".into());

        let outcome = state.handle_raw_events(vec![RawInputEvent::Key(
            crate::input::TerminalKey::new(KeyCode::Char(code), modifiers),
        )]);

        assert!(
            outcome.actions.is_empty(),
            "{code} must not target Local or Remote"
        );
        assert!(outcome.requests.is_empty());
        assert!(state.pending_requests.is_empty());
        assert!(state.overlay.is_none());
        assert_eq!(state.mode, ClientShellMode::Navigate);
        assert_eq!(state.navigate_endpoint_id.as_ref(), Some(&remote_id));
        assert!(state.visible_endpoint_notice.is_some());
        let activate = state.handle_input_bytes(b"\r");
        assert!(matches!(
            activate.actions.as_slice(),
            [ClientShellAction::ActivateEndpoint { endpoint_id, .. }] if endpoint_id == &remote_id
        ));
    }
}

#[test]
fn direct_focus_left_enters_sidebar_at_the_pane_boundary() {
    let mut config = Config::default();
    config.keys.focus_pane_left = crate::config::BindingConfig::one("ctrl+h");
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&config));
    state.set_snapshot(Box::new(sidebar_navigation_snapshot()));
    state.set_pane_surface(surface());
    state.compose(106, 30).expect("initial frame geometry");

    let enter = state.handle_raw_events(vec![RawInputEvent::Key(crate::input::TerminalKey::new(
        KeyCode::Char('h'),
