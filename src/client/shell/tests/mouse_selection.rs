...
#[test]
fn attention_title_and_dismiss_clicks_dispatch_named_actions() {
    let build = || {
        ...
        state.set_endpoint_attention(
            ...
            Some(crate::protocol::endpoint::EndpointAttentionProjection {
                pane_id: "pane_1".into(),
                source_workspace_id: "ws_source".into(),
                source_tab_id: "tab_source".into(),
        ...
        pane_surface.frame =
            FrameData::from_ratatui_buffer(&Buffer::empty(Rect::new(0, 0, 40, 5)), None);
    ...
    let mut title_state = build();
    let pane = title_state.hits.panes[0].clone();
    let title = title_state.handle_raw_events(vec![RawInputEvent::Mouse(MouseEvent {
        kind: MouseEventKind::Down(MouseButton::Left),
        column: pane.rect.x + 2,
        row: pane.rect.y,
        ...
        [ClientShellAction::Endpoint { request, .. }]
            if matches!(
                ...
                crate::api::schema::Method::AttentionOpen(params)
                    if params.source_pane_id == "pane_source"
            )
    ));

    let mut dismiss_state = build();
    let pane = dismiss_state.hits.panes[0].clone();
    let dismiss = dismiss_state.handle_raw_events(vec![RawInputEvent::Mouse(MouseEvent {
        kind: MouseEventKind::Down(MouseButton::Left),
        column: pane.rect.right() - 2,
        row: pane.rect.y,
        modifiers: KeyModifiers::NONE,
    })]);
    assert!(matches!(
...
#[test]
fn borderless_attention_pane_leaves_first_row_mouse_input_to_terminal() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(snapshot()));
    state.set_endpoint_methods(Some(vec![
        crate::protocol::endpoint::ATTENTION_OPEN_METHOD.into(),
    ...
    state.set_pane_surface(pane_surface);
    state.compose(106, 20).expect("borderless pane frame");
