...
#[test]
fn remote_attention_actions_stay_endpoint_qualified() {
    let (mut state, remote) = state_with_remote();
    state.set_endpoint_methods_for(
        &remote,
        Some(vec![crate::protocol::endpoint::ATTENTION_OPEN_METHOD.into()]),
    );
    state.set_endpoint_attention(
        ...
        Some(crate::protocol::endpoint::EndpointAttentionProjection {
            ...
            source_tab_id: "tab_1".into(),
            source_pane_id: "source-pane".into(),
            title: "pi · remote-workspace".into(),
        }),
    );
    ...
    let [ClientShellAction::Endpoint {
        ...
        panic!("expected remote attention action");
    };
...
#[test]
fn sidebar_navigation_targets_remain_endpoint_qualified_with_colliding_ids() {
    let (mut state, remote) = state_with_remote();
    let mut remote_snapshot = state
        ...
        .clone();
    remote_snapshot.agents = vec![agent("remote", AgentStatus::Blocked, 1)];
    state.set_endpoint_snapshot(&remote, Box::new(remote_snapshot));
    let mut local = snapshot();
    local.agents = vec![agent("local", AgentStatus::Idle, 2)];
    state.set_snapshot(Box::new(local));
    state.mode = ClientShellMode::Navigate;
    state.select_navigate_agent(remote.clone(), "pane_1".into());
