...
#[test]
fn attention_overlay_hides_host_graphics_and_restores_retained_images_on_close() {
    ...
    state.set_attention_queue(
        ...
        vec![crate::protocol::endpoint::EndpointAttentionEntry {
            pane_id: "source-agent".into(),
            source_workspace_id: "source-workspace".into(),
            source_tab_id: "source-tab".into(),
            title: "Waiting agent".into(),
            kind: crate::protocol::endpoint::EndpointAttentionKind::Blocked,
        }],
    );
    ...
    state.set_pane_surface(surface);
    assert!(is_placed(&state.compose(106, 40).unwrap().graphics, point));

    state.handle_input_bytes(&[0x02]);
    state.handle_input_bytes(b"o");
    assert!(state.attention_open());
    let covered = state.compose(106, 40).unwrap();
    assert!(!is_placed(&covered.graphics, point));
    assert!(String::from_utf8_lossy(&covered.graphics).contains("a=d,d=i"));

    state.handle_input_bytes(&[0x02]);
    state.handle_input_bytes(b"o");
    assert!(!state.attention_open());
    let restored = state.compose(106, 40).unwrap();
    assert!(is_placed(&restored.graphics, point));
