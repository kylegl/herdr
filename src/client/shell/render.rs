        ...
        match mode {
            ...
            ClientShellMode::Navigate => {
                segments.extend([
                    ...
                    ("h/j/k/l".to_owned(), key),
                    (" sections/pane  ".to_owned(), base),
                    ("enter".to_owned(), key),
                    (" focus  ".to_owned(), base),
                    (prefix_rhs(&keybinds.keybinds.help), key),
                    (" keybinds".to_owned(), base),
...
pub(super) struct ShellRenderState<'a> {
    ...
    pub(super) selected_endpoint_id: Option<&'a ClientEndpointId>,
        ...
        if state.endpoints.len() > 1 {
            ...
            render_collapsed_sidebar(
                ...
                state
                    ...
                    .is_none_or(|endpoint| endpoint == state.active_endpoint_id)
                    .then_some(state.selected_workspace_id)
                    .flatten(),
                state
                    .selected_endpoint_id
                    .is_none_or(|endpoint| endpoint == state.active_endpoint_id)
                    .then_some(state.selected_agent_pane_id)
                    .flatten(),
                &mut hits,
            );
        } else {
