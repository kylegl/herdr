        ...
        match binding {
            ...
            crate::input::KeybindMatch::Action(action) => {
                ...
                if action == crate::input::KeybindAction::OpenNotificationTarget {
                    let opens_attention = self.active_attention().is_some_and(|attention| {
                        self.snapshot
                            .as_deref()
                            .and_then(|snapshot| snapshot.focused_pane_id.as_deref())
                            == Some(attention.pane_id.as_str())
                    });
                    if opens_attention {
                        if let Some(attention) = self.active_attention() {
                ...
                if action == crate::input::KeybindAction::DismissAttention {
                    if let Some(attention) = self.active_attention() {
                        self.push_endpoint_method(
