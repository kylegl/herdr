...
pub(super) fn render_collapsed(
    ...
    selected_endpoint_id: Option<&ClientEndpointId>,
    ...
    for (index, row) in rows.into_iter().take(area.height as usize).enumerate() {
        ...
        let selected = selected_endpoint_id == Some(&row.endpoint_id)
            && selected_pane_id == Some(row.agent.pane_id.as_str());
        if selected {
...
pub(super) fn render_expanded(
    ...
    super::agent_sidebar::render_agent_list(
        ...
        config,
        agent_scroll,
        selected_endpoint_id.and_then(|selected_endpoint| {
            selected_pane_id.and_then(|selected_pane| {
                rows.iter().position(|row| {
                    selected_endpoint == &row.endpoint_id
                        && selected_pane == row.agent.pane_id.as_str()
                })
            })
        }),
        hits,
        ...
        |buffer, rect, row, hits| {
            ...
            if selected_endpoint_id == Some(&row.endpoint_id)
                && selected_pane_id == Some(row.agent.pane_id.as_str())
            {
                buffer.set_style(rect, Style::default().bg(config.palette.selection_bg));
            }
