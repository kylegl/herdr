...
pub(crate) struct ClientShellEndpoint {
    ...
    pub(crate) attention: Option<crate::protocol::endpoint::EndpointAttentionProjection>,
...
impl ClientShellState {
    pub(crate) fn set_endpoint_catalog(&mut self, profiles: &[SavedSshEndpoint]) {
        ...
        for profile in profiles {
            ...
            next.push(ClientShellEndpoint {
                ...
                attention: previous.and_then(|endpoint| endpoint.attention.clone()),
    ...
    pub(crate) fn set_endpoint_attention(
        &mut self,
        endpoint_id: &ClientEndpointId,
        ...
        if let Some(endpoint) = self
            ...
            .find(|endpoint| &endpoint.endpoint_id == endpoint_id)
        {
            endpoint.attention = attention;
        }
    }

    pub(super) fn active_attention(
        &self,
    ) -> Option<&crate::protocol::endpoint::EndpointAttentionProjection> {
        self.endpoints
            .iter()
            .find(|endpoint| endpoint.endpoint_id == self.active_endpoint_id)
            .and_then(|endpoint| endpoint.attention.as_ref())
    }
