...
impl ClientShellState {
        ...
        match self.mode {
            ClientShellMode::Terminal => {
                    ...
                    crate::input::resolve_direct_binding(&self.config.keybinds.keybinds, key)
                {
                    ...
                    ) && self.enter_sidebar_navigation_from_left_boundary()
                    {
                        outcome.repaint = true;
                        return None;
                    }
    ...
    pub(super) fn route_navigate_key(
        &mut self,
        key: &crate::input::TerminalKey,
        outcome: &mut ClientShellInput,
    ) {
        ...
        {
            if self.navigate_agent_pane_id.is_some() {
        ...
        if modifiers.is_empty() {
            match code {
                KeyCode::Enter => {
                    let endpoint_id = self.navigate_selection_endpoint().clone();
                    let target = if let Some(pane_id) = self.navigate_agent_pane_id.clone() {
                        Some(ClientEndpointFocusTarget::Pane(pane_id))
                    } else {
                ...
                KeyCode::Right if self.sidebar_section_focus => {
                    self.mode = self.copy_or_terminal_mode();
                    self.clear_navigate_selection();
                    outcome.repaint = true;
                    return;
                }
                KeyCode::Left | KeyCode::Right if !self.sidebar_section_focus => {
                    let action = if code == KeyCode::Left {
        ...
        if let Some(action) = pane_action {
            if self.sidebar_section_focus {
                match action {
                    ...
                    KeybindAction::FocusPaneDown if self.navigate_agent_pane_id.is_none() => {
                        self.select_focused_navigate_agent();
                        outcome.repaint = true;
                    }
                    KeybindAction::FocusPaneUp if self.navigate_agent_pane_id.is_some() => {
                        self.select_focused_navigate_workspace();
                        outcome.repaint = true;
                    }
                    _ => self.record_navigate_binding(KeybindMatch::Action(action), true, outcome),
                }
            } else {
                self.record_navigate_binding(KeybindMatch::Action(action), true, outcome);
            }
    ...
    fn record_navigate_binding(
        ...
        if self.navigate_selection_endpoint() != &self.active_endpoint_id
            && matches!(
                binding,
                KeybindMatch::Action(
        ...
        {
            outcome.repaint |= self.receive_endpoint_unavailable(
                "Press Enter to activate the selected endpoint before workspace actions".into(),
            );
            return;
        }
    ...
    fn move_navigate_workspace(&mut self, delta: isize) {
        ...
        if mobile {
            ...
            let current = self
                ...
                .unwrap_or(0);
            let next = (current as isize + delta).clamp(0, entries.len().saturating_sub(1) as isize)
                as usize;
            let workspace_id = snapshot.workspaces[entries[next].index]
                .workspace_id
                .clone();
            self.select_navigate_workspace(self.active_endpoint_id.clone(), workspace_id);
            self.reveal_mobile_workspace = true;
            return;
        }

        let targets = self.sidebar_navigation_workspace_targets();
        if targets.is_empty() {
            return;
        }
        let selected_endpoint = self.navigate_selection_endpoint();
        let current = self
            .navigate_workspace_id
            .as_deref()
            .and_then(|selected| {
                targets.iter().position(|target| {
                    &target.endpoint_id == selected_endpoint && target.workspace_id == selected
                })
            })
            .unwrap_or(0);
        let next = (current as isize + delta).rem_euclid(targets.len() as isize) as usize;
        let target = targets[next].clone();
        self.select_navigate_workspace(target.endpoint_id.clone(), target.workspace_id.clone());
        self.reveal_workspace_on_endpoint(&target.endpoint_id, &target.workspace_id);
    }
    ...
    fn move_navigate_agent(&mut self, delta: isize) {
        ...
        let current = self
            ...
            .and_then(|selected| {
                targets.iter().position(|target| {
                    &target.endpoint_id == selected_endpoint && target.pane_id == selected
                })
            })
            .unwrap_or(0);
        let next = (current as isize + delta).clamp(0, targets.len() as isize - 1) as usize;
        let target = targets[next].clone();
        self.select_navigate_agent(target.endpoint_id, target.pane_id);
        self.agent_scroll = next.min(self.hits.agent_max_scroll);
    }

    fn select_focused_navigate_agent(&mut self) {
        let targets = super::aggregate_navigation::online_agent_targets(
            &self.endpoints,
            self.config.agent_panel_sort,
        );
        ...
        if let Some(target) = targets.get(index) {
            self.select_navigate_agent(target.endpoint_id.clone(), target.pane_id.clone());
            self.agent_scroll = index.min(self.hits.agent_max_scroll);
        }
    }

    fn select_focused_navigate_workspace(&mut self) {
    ...
    fn enter_sidebar_navigation_from_left_boundary(&mut self) -> bool {
        ...
        if self.layout(cols, rows).sidebar.is_empty() {
            return false;
        }
        let (Some(snapshot), Some(surface)) =
            (self.snapshot.as_deref(), self.pane_surface.as_ref())
        else {
            return false;
        };
        let Some(focused_id) = snapshot.focused_pane_id.as_deref() else {
            return false;
        };
        let Some(focused) = surface.panes.iter().find(|pane| pane.pane_id == focused_id) else {
            return false;
        };
        let has_left_target = surface.panes.iter().any(|pane| {
            pane.pane_id != focused.pane_id
                && pane.rect.x.saturating_add(pane.rect.width) <= focused.rect.x
                && pane.rect.y < focused.rect.y.saturating_add(focused.rect.height)
                && focused.rect.y < pane.rect.y.saturating_add(pane.rect.height)
        });
        if has_left_target {
            return false;
        }
        ...
        self.select_navigate_workspace(self.active_endpoint_id.clone(), workspace_id);
        self.sidebar_section_focus = true;
        true
    }
