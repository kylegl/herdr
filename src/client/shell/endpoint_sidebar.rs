...
pub(super) fn render_collapsed(
    ...
    for (index, endpoint) in state.endpoints.iter().enumerate() {
        ...
        for workspace in &snapshot.workspaces {
            ...
            let selected = state.selected_endpoint_id == Some(&endpoint.endpoint_id)
...
pub(super) fn render_expanded(
    ...
    let selected_index = state.selected_workspace_id.and_then(|selected_workspace| {
        let selected_endpoint = state.selected_endpoint_id?;
        rows.iter().position(|row| match row {
            Row::Workspace { endpoint, entry } => {
                let endpoint = &state.endpoints[*endpoint];
                &endpoint.endpoint_id == selected_endpoint
                    && endpoint.snapshot.as_deref().is_some_and(|snapshot| {
                        snapshot
                            .workspaces
                            .get(entry.index)
                            .is_some_and(|workspace| {
                                selected_workspace == workspace.workspace_id.as_str()
                            })
                    })
            }
