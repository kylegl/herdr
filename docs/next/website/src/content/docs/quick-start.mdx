...
Herdr is mouse-native, so start by clicking panes, tabs, workspaces, and agents to focus them. Drag split borders to resize. Right-click for context menus, including splitting panes and creating tabs. The automatic attention dock temporarily presents blocked agents first and background completions second without stealing focus; see [Agents](/docs/agents/#attention-dock). Drag-select text to copy it to your clipboard; double-click a token to copy it directly. Copying does not require Ctrl+C.
