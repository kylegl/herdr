...
Attention-dock placement is temporary: snapshots save a docked pane back at its original layout position. Dock designations and the live attention queue are cleared by a full server restart and can be set again from each pane's context menu.
