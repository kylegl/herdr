...
The attention overlay does not move panes or alter saved layouts. The live attention queue is cleared by a full server restart.
...
A successful handoff preserves long-lived server-owned session state: pane PTYs and processes, agent identity and durable metadata, and plugin/session state needed by the replacement server. Between compatible fork servers, handoff also transfers attention queue order, entry kinds, and dismissals. The client reconnects its attention view separately.
