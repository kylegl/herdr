...
Attention-dock placement is temporary: snapshots omit its transient split and save the attention pane in its original layout position. The live attention queue is cleared by a full server restart.
...
A successful handoff preserves long-lived server-owned session state: pane PTYs and processes, agent identity and durable metadata, and plugin/session state needed by the replacement server. If an attention pane is temporarily placed when handoff begins, Herdr first restores canonical topology, transfers the matching PTY ownership, and reconstructs blocked and unseen-done attention entries in the replacement server.

It does not preserve transient coordination across the replacement boundary. In-flight CLI or API requests, waits, subscription streams, client sockets, and pane-to-pane messages may be interrupted; clients should reconnect and retry them.
