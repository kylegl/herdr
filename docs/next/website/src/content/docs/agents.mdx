...
Attention placement and the queue belong to the live server session and are cleared by a full server restart. Session snapshots always save the original layout without temporary splits.
