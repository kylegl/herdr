...
The sidebar's Attention row shows red and green counts for agents waiting for input and agents that finished in the background. New arrivals update the counts without opening anything. Click the row or press `prefix+o` to toggle an interactive terminal overlay. Blocked agents come first, followed by finished agents, with first-in-first-out ordering inside each group. Entries wait 300 milliseconds before becoming available.
...
Use Prev and Next to browse the queue. New arrivals do not replace the selected agent. When that agent resumes working or exits, the overlay advances to the next remaining item instead of disappearing. It closes when the queue is empty. Dismiss or `prefix+shift+o` acknowledges only the selected occurrence and continues through the queue once the server confirms removal. The same agent can reappear after it resumes and later needs attention again.

To hide the overlay without acknowledging an entry, press `prefix+o`, click outside it, or use Close. Escape goes to the agent rather than closing the overlay. Jump closes the overlay and takes you to the agent's original pane without acknowledging it.

The overlay leaves the underlying pane layout and dimensions unchanged. Each client chooses its own attention view. The selected terminal can resize to fit, but its workspace, tab, pane identity, and PTY ownership stay unchanged.

The queue belongs to the live server session. Live handoff preserves queue order and dismissals. Session snapshots contain the ordinary pane layout without temporary splits.
