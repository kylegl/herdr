...
When the expanded sidebar is beside the leftmost pane, moving left transfers keyboard focus to the workspace list. Use the configured pane directions (`h/j/k/l` by default) to move between the workspace list, agent list, and panes. In the agent list, use the workspace up/down bindings (Up/Down by default) to choose an agent and Enter to focus its pane.
